...
TEST(DictTest, update_with_empty_other_does_nothing) {
  const auto a = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{2, 3});
  const auto b = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{4, 5});
  Coords coords{a.dims(), {{Dim{"a"}, a}, {Dim{"b"}, b}}};
  const Coords other{a.dims(), {}};
  coords.update(other);
  ASSERT_EQ(coords.at(Dim{"a"}), a);
  ASSERT_EQ(coords.at(Dim{"b"}), b);
  ASSERT_EQ(coords.size(), 2);
}

TEST(DictTest, update_with_empty_this_assigns_other) {
  const auto a = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{2, 3});
  const auto b = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{4, 5});
  Coords coords{a.dims(), {}};
  const Coords other{a.dims(), {{Dim{"a"}, a}, {Dim{"b"}, b}}};
  coords.update(other);
  ASSERT_EQ(coords.at(Dim{"a"}), a);
  ASSERT_EQ(coords.at(Dim{"b"}), b);
  ASSERT_EQ(coords.size(), 2);
...
TEST(DictTest, update_with_assigns_from_other) {
  const auto a = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{2, 3});
  const auto b = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{4, 5});
  const auto c = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{6, 7});
  const auto d = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{8, 9});
  Coords coords{a.dims(), {{Dim{"a"}, a}, {Dim{"b"}, b}}};
  const Coords other{c.dims(), {{Dim{"c"}, c}, {Dim{"d"}, d}}};
  coords.update(other);
  ASSERT_EQ(coords.at(Dim{"a"}), a);
  ASSERT_EQ(coords.at(Dim{"b"}), b);
  ASSERT_EQ(coords.at(Dim{"c"}), c);
  ASSERT_EQ(coords.at(Dim{"d"}), d);
  ASSERT_EQ(coords.size(), 4);
}

TEST(DictTest, update_with_replaces_existing) {
  const auto a = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{2, 3});
  const auto b = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{4, 5});
  const auto b2 = makeVariable<double>(Dims{Dim::X}, Shape{2}, Values{6, 7});
  Coords coords{a.dims(), {{Dim{"a"}, a}, {Dim{"b"}, b}}};
  const Coords other{a.dims(), {{Dim{"b"}, b2}}};
  coords.update(other);
  ASSERT_EQ(coords.at(Dim{"a"}), a);
  ASSERT_EQ(coords.at(Dim{"b"}), b2);
  ASSERT_EQ(coords.size(), 2);
}
