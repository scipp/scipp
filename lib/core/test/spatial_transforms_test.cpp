...
TEST(SpatialTransformsTest, combine_to_linear) {
  ...
  Eigen::Matrix3d expected(Eigen::Quaterniond(
      Eigen::AngleAxisd(pi<double>, Eigen::Vector3d::UnitX())));
...
TEST(SpatialTransformsTest, combine_to_affine) {
  ...
  Eigen::Affine3d expected(Eigen::Quaterniond(
      Eigen::AngleAxisd(pi<double>, Eigen::Vector3d::UnitX())));
...
TEST(SpatialTransformsTest, combine_rotations) {
  ...
  scipp::core::Quaternion expected(Eigen::Quaterniond(
      Eigen::AngleAxisd(pi<double>, Eigen::Vector3d::UnitX())));
