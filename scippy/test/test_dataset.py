...
from scippy import Dataset, Coord, Dim, Data, Variable, Attr, split, \
    concatenate, sort, units, rebin, mean
...
class TestDataset(unittest.TestCase):
    def setUp(self):
        ...
        self.dataset[Data.Value, "data1"] = (
            [Dim.Z, Dim.Y, Dim.X], self.reference_data1)
        self.dataset[Data.Value, "data2"] = (
            [Dim.Z, Dim.Y, Dim.X], self.reference_data2)
        self.dataset[Data.Value, "data3"] = (
            [Dim.Z, Dim.X], self.reference_data3)
    ...
    def test_insert_default_init(self):
        ...
        np.testing.assert_array_equal(
            d[Data.Value, "data1"].numpy, np.zeros(shape=(4, 3, 2)))

    def test_insert(self):
        d = Dataset()
        d[Data.Value, "data1"] = (
            [Dim.Z, Dim.Y, Dim.X], np.arange(24).reshape(4, 3, 2))
        self.assertEqual(len(d), 1)
        np.testing.assert_array_equal(
            d[Data.Value, "data1"].numpy, self.reference_data1)
    ...
    def test_replace(self):
        d = Dataset()
        d[Data.Value, "data1"] = (
            [Dim.Z, Dim.Y, Dim.X], np.zeros(24).reshape(4, 3, 2))
        d[Data.Value, "data1"] = (
            [Dim.Z, Dim.Y, Dim.X], np.arange(24).reshape(4, 3, 2))
        self.assertEqual(len(d), 1)
        np.testing.assert_array_equal(
    ...
    def test_demo_int_to_float_issue(self):
        ...
        d[Data.Value, "v1"] = ([Dim.X, Dim.Y], np.ndarray.tolist(
            np.arange(0, 10).reshape(2, 5)))
        # Correct behaviour should be int64
        self.assertEqual(d[Data.Value, "v1"].numpy.dtype, 'float64')

        # Demo working 1D
        d = Dataset()
        d[Data.Value, "v2"] = ([Dim.X], np.ndarray.tolist(
    ...
    def test_set_data_nested(self):
        ...
        self.assertNotEqual(d[Data.Value, "data1"].data[0],
                            d[Data.Value, "data1"].data[1])
    ...
    def test_data(self):
        ...
        self.assertSequenceEqual(
            self.dataset[Data.Value, "data1"].data, range(24))

    def test_numpy_data(self):
        np.testing.assert_array_equal(
            self.dataset[Coord.X].numpy, self.reference_x)
        np.testing.assert_array_equal(
            self.dataset[Coord.Y].numpy, self.reference_y)
        np.testing.assert_array_equal(
            self.dataset[Coord.Z].numpy, self.reference_z)
        np.testing.assert_array_equal(
            self.dataset[Data.Value, "data1"].numpy, self.reference_data1)
        np.testing.assert_array_equal(
            self.dataset[Data.Value, "data2"].numpy, self.reference_data2)
        np.testing.assert_array_equal(
            self.dataset[Data.Value, "data3"].numpy, self.reference_data3)
    ...
    def test_insert_subdata(self):
        ...
        self.assertTrue(np.array_equal(
            d4[Data.Value, "2a"].numpy, ds_slice[Data.Value, "a"].numpy * 2))
    ...
    def test_slice_dataset(self):
        for x in range(2):
            ...
            self.assertRaisesRegex(
                ...
                Coord.X)
            np.testing.assert_array_equal(
                view[Coord.Y].numpy, self.reference_y)
            np.testing.assert_array_equal(
                view[Coord.Z].numpy, self.reference_z)
            np.testing.assert_array_equal(
                view[Data.Value, "data1"].numpy, self.reference_data1[:, :, x])
            np.testing.assert_array_equal(
                view[Data.Value, "data2"].numpy, self.reference_data2[:, :, x])
            np.testing.assert_array_equal(
                view[Data.Value, "data3"].numpy, self.reference_data3[:, x])
        for y in range(3):
            view = self.dataset[Dim.Y, y]
            np.testing.assert_array_equal(
                view[Coord.X].numpy, self.reference_x)
            self.assertRaisesRegex(
                RuntimeError,
            ...
            np.testing.assert_array_equal(
                view[Coord.Z].numpy, self.reference_z)
            np.testing.assert_array_equal(
                view[Data.Value, "data1"].numpy, self.reference_data1[:, y, :])
            np.testing.assert_array_equal(
                view[Data.Value, "data2"].numpy, self.reference_data2[:, y, :])
            np.testing.assert_array_equal(
                view[Data.Value, "data3"].numpy, self.reference_data3)
        for z in range(4):
            ...
            self.assertRaisesRegex(
                ...
                Coord.Z)
            np.testing.assert_array_equal(
                view[Data.Value, "data1"].numpy, self.reference_data1[z, :, :])
            np.testing.assert_array_equal(
                view[Data.Value, "data2"].numpy, self.reference_data2[z, :, :])
            np.testing.assert_array_equal(
                view[Data.Value, "data3"].numpy, self.reference_data3[z, :])
        for x in range(2):
            for delta in range(3 - x):
                view = self.dataset[Dim.X, x:x + delta]
                np.testing.assert_array_equal(
                    view[Coord.X].numpy, self.reference_x[x:x + delta])
                np.testing.assert_array_equal(
                    view[Coord.Y].numpy, self.reference_y)
                np.testing.assert_array_equal(
                    view[Coord.Z].numpy, self.reference_z)
                np.testing.assert_array_equal(
                    view[Data.Value, "data1"].numpy,
                    self.reference_data1[:, :, x:x + delta])
                np.testing.assert_array_equal(
                    view[Data.Value, "data2"].numpy,
                    self.reference_data2[:, :, x:x + delta])
                np.testing.assert_array_equal(
                    view[Data.Value, "data3"].numpy,
        ...
        for y in range(3):
            for delta in range(4 - y):
                ...
                np.testing.assert_array_equal(
                    view[Coord.X].numpy, self.reference_x)
                np.testing.assert_array_equal(
                    view[Coord.Y].numpy, self.reference_y[y:y + delta])
                np.testing.assert_array_equal(
                    view[Coord.Z].numpy, self.reference_z)
                np.testing.assert_array_equal(
                    view[Data.Value, "data1"].numpy,
        ...
        for z in range(4):
            for delta in range(5 - z):
                ...
                np.testing.assert_array_equal(
                    view[Coord.Y].numpy, self.reference_y)
                np.testing.assert_array_equal(
                    view[Coord.Z].numpy, self.reference_z[z:z + delta])
                np.testing.assert_array_equal(
                    view[Data.Value, "data1"].numpy,
    ...
    def test_binary_dataset_rhs_operations(self):
        ...
        self.assertTrue(np.array_equal(
            c[Data.Variance, "i"].numpy, variance + variance))
        ...
        self.assertTrue(np.array_equal(c[Data.Value, "i"].numpy, data * data))
        self.assertTrue(np.array_equal(
            c[Data.Variance, "i"].numpy, variance * (data * data) * 2))
        ...
        np.testing.assert_equal(
            c[Data.Variance, "i"].numpy, 2 * variance / (data * data))
    ...
    def test_numpy_interoperable(self):
        ...
        self.dataset[Data.Value, 'data2'] = np.exp(
            self.dataset[Data.Value, 'data1'])
        np.testing.assert_array_equal(
            self.dataset[Data.Value, "data2"].numpy,
    ...
    def test_slice_numpy_interoperable(self):
        ...
        self.dataset.subset['data2'][Data.Value, 'data2'] = np.exp(
            self.dataset[Data.Value, 'data1'])
        np.testing.assert_array_equal(
            self.dataset[Data.Value, "data2"].numpy,
            np.exp(self.reference_data1))
        # Slice view of dataset then view single variable
        self.dataset[Dim.X, 0][Data.Value, 'data2'] = np.exp(
            self.dataset[Dim.X, 1][Data.Value, 'data1'])
        np.testing.assert_array_equal(
            self.dataset[Data.Value, "data2"].numpy[..., 0],
            np.exp(self.reference_data1[..., 1]))
        # View single variable then slice view
        self.dataset[Data.Value, 'data2'][Dim.X, 1] = np.exp(
            self.dataset[Data.Value, 'data1'][Dim.X, 0])
        np.testing.assert_array_equal(
            self.dataset[Data.Value, "data2"].numpy[..., 1],
            np.exp(self.reference_data1[..., 0]))
        # View single variable then view range of slices
        self.dataset[Data.Value, 'data2'][Dim.Y, 1:3] = np.exp(
            self.dataset[Data.Value, 'data1'][Dim.Y, 0:2])
        np.testing.assert_array_equal(self.dataset[Data.Value,
                                      "data2"].numpy[:, 1:3, :],
    ...
    def test_split(self):
        ...
        np.testing.assert_array_equal(
            d0[Data.Value, "data"].numpy, np.array([0, 1]))
        d1 = datasets[1]
        np.testing.assert_array_equal(d1[Coord.X].numpy, np.array([4, 1]))
        np.testing.assert_array_equal(
            d1[Data.Value, "data"].numpy, np.array([2, 3]))
    ...
    def test_concatenate(self):
        ...
        np.testing.assert_array_equal(
            dataset[Coord.X].numpy, np.array([3, 2, 4, 1, 3, 2, 4, 1]))
        np.testing.assert_array_equal(
            dataset[Data.Value, "data"].numpy,
    ...
    def test_concatenate_with_slice(self):
        ...
        np.testing.assert_array_equal(
            dataset[Coord.X].numpy, np.array([3, 2, 4, 1, 3, 2]))
        np.testing.assert_array_equal(
            dataset[Data.Value, "data"].numpy, np.array([0, 1, 2, 3, 0, 1]))
    ...
    def test_rebin(self):
        ...
        np.testing.assert_array_equal(
            dataset[Data.Value, "data"].numpy, np.array(5 * [2]))
        np.testing.assert_array_equal(
            dataset[Coord.X].numpy, np.arange(0, 11, 2))
    ...
    def test_sort(self):
        ...
        np.testing.assert_array_equal(
            dataset[Data.Value, "data"].numpy, np.array([3, 1, 0, 2]))
        np.testing.assert_array_equal(
            dataset[Coord.X].numpy, np.array([1, 2, 3, 4]))
        dataset = sort(dataset, Data.Value, "data")
        np.testing.assert_array_equal(
            dataset[Data.Value, "data"].numpy, np.arange(4))
        np.testing.assert_array_equal(
            dataset[Coord.X].numpy, np.array([3, 2, 4, 1]))

    def test_filter(self):
        dataset = Dataset()
        dataset[Data.Value, "data"] = ([Dim.X], np.arange(4))
        dataset[Coord.X] = ([Dim.X], np.array([3, 2, 4, 1]))
        select = Variable(Coord.Mask, [Dim.X], np.array(
            [False, True, False, True]))
        dataset = filter(dataset, select)
        np.testing.assert_array_equal(
            dataset[Data.Value, "data"].numpy, np.array([1, 3]))
...
class TestDatasetExamples(unittest.TestCase):
    def test_table_example(self):
        ...
        self.assertSequenceEqual(table[Coord.Row].data, [
                                 'a', 'bb', 'ccc', 'dddd'])
        table[Data.Value, "col1"] = ([Dim.Row], [3.0, 2.0, 1.0, 0.0])
        table[Data.Value, "col2"] = ([Dim.Row], np.arange(4.0))
        ...
        np.testing.assert_array_equal(
            table[Data.Value, "col2"].numpy, np.array([0, 1, 2, 3]))
        np.testing.assert_array_equal(
            table[Data.Value, "sum"].numpy, np.array([6, 6, 6, 6]))

        table = concatenate(table, table, Dim.Row)
        np.testing.assert_array_equal(
            table[Data.Value, "col1"].numpy,
            np.array([3, 2, 1, 0, 3, 2, 1, 0]))

        table = concatenate(table[Dim.Row, 0:2], table[Dim.Row, 5:7], Dim.Row)
        np.testing.assert_array_equal(
            table[Data.Value, "col1"].numpy, np.array([3, 2, 2, 1]))

        table = sort(table, Data.Value, "col1")
        np.testing.assert_array_equal(
            table[Data.Value, "col1"].numpy, np.array([1, 2, 2, 3]))
        ...
        np.testing.assert_array_equal(
            table[Data.Value, "col1"].numpy, np.array([3, 5, 7, 8]))

        table[Data.Value, "exp1"] = (
            [Dim.Row], np.exp(table[Data.Value, "col1"]))
        table[Data.Value, "exp1"] -= table[Data.Value, "col1"]
        np.testing.assert_array_equal(table[Data.Value, "exp1"].numpy, np.exp(
            np.array([3, 5, 7, 8])) - np.array([3, 5, 7, 8]))
    ...
    def test_MDHistoWorkspace_example(self):
        ...
        d[Data.Value, "temperature"] = (
            [Dim.Z, Dim.Y, Dim.X],
            np.random.normal(size=L * L * L).reshape([L, L, L]))
        d[Data.Value, "pressure"] = (
            [Dim.Z, Dim.Y, Dim.X],
            np.random.normal(size=L * L * L).reshape([L, L, L]))
        # Add uncertainties, matching name implicitly links it to corresponding
        # data
        d[Data.Variance, "temperature"] = (
            [Dim.Z, Dim.Y, Dim.X], d[Data.Value, "temperature"].numpy)
    ...
    def test_Workspace2D_example(self):
        ...
        select = Variable(Coord.Mask, [Dim.Spectrum], np.isin(
            d[Coord.SpectrumNumber], np.arange(10, 20)))
    ...
    def test_monitors_example(self):
        ...
        status[Data.PulseTime] = (
            [Dim.Event], np.random.exponential(size=1000))
        ...
        transmission[Data.Value] = (
            [Dim.Energy], np.random.exponential(size=8))
    ...
    def test_zip(self):
        ...
        # for el in zip(d[Data.EventTofs, ""].data, d[Data.EventPulseTimes,
        # ""].data):
    ...
    def test_rebin(self):
        ...
        rd1 = rebin(d1, Variable(Coord.X, [Dim.X], np.arange(
