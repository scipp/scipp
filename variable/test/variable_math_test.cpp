...
TEST(Variable, exp_bad_unit) {
  EXPECT_THROW_DROP_RES(exp(0.0 * units::s), except::UnitError);
}
...
TEST(Variable, log_bad_unit) {
  EXPECT_THROW_DROP_RES(log(1.0 * units::s), except::UnitError);
}
...
TEST(Variable, log10_bad_unit) {
  EXPECT_THROW_DROP_RES(log10(1.0 * units::s), except::UnitError);
}
