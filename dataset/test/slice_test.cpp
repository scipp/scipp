...
TEST(SliceTest, test_dimension_not_found) {
  ...
  EXPECT_THROW(auto s = dataset::slice(da, Dim::Y),
...
TEST(SliceTest, test_slice_range_on_point_coords_1D_descending) {
  ...
  auto da = make_1d_data_array(12, 2, Dim::X, CoordType::Points);
  // No effect slicing
  auto out = slice(da, Dim::X, 12.0 * units::dimensionless,
                   2.0 * units::dimensionless);
  EXPECT_EQ(da, out);
  // Test start on left boundary (closed on left), so includes boundary
  out = slice(da, Dim::X, 12.0 * units::dimensionless,
              11.0 * units::dimensionless);
  EXPECT_EQ(out, da.slice({Dim::X, 0, 1}));
  // Test start out of bounds on left truncated
  out = slice(da, Dim::X, 13.0 * units::dimensionless,
              11.0 * units::dimensionless);
  EXPECT_EQ(out, da.slice({Dim::X, 0, 1}));
  // Test inner values
  out = slice(da, Dim::X, 11.5 * units::dimensionless,
              9.5 * units::dimensionless);
  EXPECT_EQ(out, da.slice({Dim::X, 1, 3}));
  // Test end on right boundary (open on right), so does not include boundary
  out =
      slice(da, Dim::X, 4.0 * units::dimensionless, 3.0 * units::dimensionless);
  EXPECT_EQ(out, da.slice({Dim::X, 8, 9}));
  // Test end out of bounds on right truncated
  out =
      slice(da, Dim::X, 4.0 * units::dimensionless, 1.0 * units::dimensionless);
  EXPECT_EQ(out, da.slice({Dim::X, 8, 10}));
}
...
TEST(SliceTest, test_slice_range_on_edge_coords_1D_ascending) {
  ...
  // test)
  out =
      slice(da, Dim::X, 4.0 * units::dimensionless, 6.0 * units::dimensionless);
  EXPECT_EQ(out, da.slice({Dim::X, 1, 3}));
  // Test end on right boundary (open on right), so does not include boundary
  out = slice(da, Dim::X, 11.0 * units::dimensionless,
              12.0 * units::dimensionless);
...
TEST(SliceTest, test_slice_range_on_edge_coords_1D_descending) {
  ...
  EXPECT_EQ(out, da.slice({Dim::X, 0, 1}));
  // Test slicing with range boundary inside edge, same result as above expected
  slice(da, Dim::X, 11.9 * units::dimensionless, 11.0 * units::dimensionless);
  EXPECT_EQ(out, da.slice({Dim::X, 0, 1}));
  // Test slicing with range lower boundary on upper edge of bin (open on right
  // test)
  out = slice(da, Dim::X, 11.0 * units::dimensionless,
              9.0 * units::dimensionless);
  EXPECT_EQ(out, da.slice({Dim::X, 1, 3}));
  // Test end on right boundary (open on right), so does not include boundary
  out =
      slice(da, Dim::X, 4.0 * units::dimensionless, 3.0 * units::dimensionless);
...
TEST(SliceTest, test_point_on_point_coords_1D_ascending) {
  ...
  begin = 12.1 * units::dimensionless;
  EXPECT_THROW(auto s = slice(da, Dim::X, begin, begin), except::NotFoundError);
}
...
TEST(SliceTest, test_point_on_point_coords_1D_descending) {
  ...
  // No effect slicing
  // Test start on left boundary (closed on left), so includes boundary
  auto begin = 12.0 * units::dimensionless;
  auto out = slice(da, Dim::X, begin, begin);
  EXPECT_EQ(out, da.slice({Dim::X, 0}));
  // Test point slice between points throws
  begin = 3.5 * units::dimensionless;
  EXPECT_THROW(slice(da, Dim::X, begin, begin), except::NotFoundError);
  // Test start on right boundary
  begin = 3.0 * units::dimensionless;
  out = slice(da, Dim::X, begin, begin);
  EXPECT_EQ(out, da.slice({Dim::X, 9}));
  // Test start outside right boundary throws
