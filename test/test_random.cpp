...
Random::make_variable(const scipp::Dimensions &dims, const scipp::core::DType dtype,
  ...
  if (dtype == core::dtype<int64_t>) {
    return makeVariable<int64_t>(
        dims, Values(generate<int64_t>(dims.volume())), unit);
  }
  if (dtype == core::dtype<int32_t>) {
    return makeVariable<int32_t>(
        dims, Values(generate<int32_t>(dims.volume())), unit);
  }
...
scipp::Variable makeRandom(const scipp::Dimensions &dims,
                           const double min,
                           const double max) {
using namespace scipp;
Random rand(min, max);
