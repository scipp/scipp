...
:: Benchmarks
cmake --build . --target all-benchmarks --config Release || echo ERROR && exit /b 1

:: C++ tests
cmake --build . --target all-tests --config Release|| echo ERROR && exit /b 1
bin\Release\scipp-common-test.exe || echo ERROR && exit /b 1
bin\Release\scipp-core-testss.exe || echo ERROR && exit /b 1
...
cmake --build . --target install --config Release|| echo ERROR && exit /b 1
