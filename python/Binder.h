...
template <class ObjToBindType>
class Binder {
  struct PriorityFunction {
    std::function<void(ObjToBindType &obj)> bindFunction;
    Priority priority;
    bool operator<(const PriorityFunction& other) const { return priority < other.priority; }
  };
public:
  template <class...TS>
  Binder(TS&&...args) : object(std::forward<TS>(args)...) {};
  void append(PriorityFunction func) {
    bindings.emplace(std::move(func));
  };
  ObjToBindType& bind() {
