...
void init_operations(py::module &m) {
  ...
  docs.set_description("Element-wise minimum over all dimensions.");
  docs.erase_param(1);
  bind_free_function<Variable, VAConstView>(min, "min", m, docs);
  ...
  docs.set_description("Element-wise maximum over all dimensions.");
  docs.erase_param(1);
  bind_free_function<Variable, VAConstView>(max, "max", m, docs);
