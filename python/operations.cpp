...
void init_operations(py::module &m) {
  ...
  bind_free_function<Variable, VAConstView, const Dim>(flatten, "flatten", m,
                                                       docs);
  bind_free_function<DataArray, DAConstView, const Dim>(flatten, "flatten", m,
                                                        docs);
  bind_free_function<Dataset, DSConstView, const Dim>(flatten, "flatten", m,
                                                      docs);
  ...
  docs = {
    ...
    In the case of a dataset, the output contains only items that are present in both inputs.)",
      // Raises
      "If the dtype or unit does not match, or if the dimensions and shapes "
      "are incompatible.",
      // See also
      ":py:class:`scipp.sum`",
      // Returns
  ...
  bind_free_function<Variable, VAConstView, VAConstView, const Dim>(
      concatenate, "concatenate", m, docs);
  bind_free_function<DataArray, DAConstView, DAConstView, const Dim>(
      concatenate, "concatenate", m, docs);
  bind_free_function<Dataset, DSConstView, DSConstView, const Dim>(
      concatenate, "concatenate", m, docs);

  // Dot-product
  docs = {"Element-wise dot-product.",
          "If the dtype is not a vector such as "
          ":py:class:`scipp.dtype.vector_3_double.`",
          "",
  ...
  // Sort
  docs = {"Sort variable along a dimension by a sort key.",
          "If the key is invalid, e.g., if it has not exactly one dimension, "
  ...
  bind_free_function<DataArray, DAConstView, VAConstView>(sort, "sort", m,
                                                          docs);
  bind_free_function<Dataset, DSConstView, VAConstView>(sort, "sort", m, docs);
  docs.set_description("Sort data array along a dimension by the coordinate "
  ...
  bind_free_function<DataArray, DAConstView, const Dim &>(sort, "sort", m,
                                                          docs);
  bind_free_function<Dataset, DSConstView, const Dim &>(sort, "sort", m, docs);

  // Absolute value
  docs = {"Element-wise absolute value.",
  ...
  bind_free_function<VAView, VAConstView, const VAView &>(
      reciprocal, "reciprocal", m, docs.with_out_arg());

  // Norm
  docs = {"Element-wise norm.",
  ...
  // All
  docs = {"Element-wise AND over the specified dimension.",
  ...
  // Any
  docs = {"Element-wise OR over the specified dimension.",
          "If the dimension does not exist, or if the dtype is not bool.",
          ":py:class:`scipp.all`.",
          "Variable, data array, or dataset containing the reduced values.",
          "Variable, DataArray, or Dataset.",
  ...
  // Min
  docs = {"Element-wise minimum over the specified dimension.",
          "",
          ":py:class:`scipp.max`.",
  ...
  bind_free_function<bool, VAConstView>(contains_events, "contains_events", m,
                                        docs);
  bind_free_function<bool, DAConstView>(contains_events, "contains_events", m,
                                        docs);
