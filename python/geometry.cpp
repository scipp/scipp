...
void bind_component(T (*func)(T1), std::string comp, py::module &m) {
  Docstring docs = {
    // Description
    "Un-zip functionality to produce a Variable of the " + comp + " component of a vector_3_float64.",
    // Raises
    "If the dtype of input is not vector_3_float64.",
    // See also
    ":py:class:`scipp.geometry.x`, `scipp.geometry.y`, :py:class:`scipp.geometry.z`.",
    // Returns
...
void init_geometry(py::module &m) {
  ...
  bind_free_function<Variable, ConstView, ConstView, ConstView>(position, "position", geom_m, docs);
  ...
  docs = {
    ...
    // Raises
    "If the units of the inputs are not meter and dimensionless, for the positions and the rotations, respectively.",
    // See also
    "",
    // Returns
  ...
  bind_free_function<Variable, ConstView, ConstView>(rotate, "rotate", geom_m, docs);
  bind_free_function<View, ConstView, ConstView, const View &>(rotate, "rotate", geom_m, docs.with_out_arg());
