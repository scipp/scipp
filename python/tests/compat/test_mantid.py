...
class TestMantidConversion(unittest.TestCase):
    ...
    def test_mdhisto_workspace(self):
        ...
        histo_dataarray = mantidcompat.load(file_name)

        self.assertEqual(histo_dataarray.coords[sc.Dim.X].values.shape, (3, ))
        self.assertEqual(histo_dataarray.coords[sc.Dim.Y].values.shape, (4, ))
        self.assertEqual(histo_dataarray.coords[sc.Dim.Z].values.shape, (5, ))
        self.assertEqual(histo_dataarray.coords[sc.Dim.X].unit,
                         sc.units.dimensionless)
        self.assertEqual(histo_dataarray.coords[sc.Dim.Y].unit,
                         sc.units.dimensionless)
        self.assertEqual(histo_dataarray.coords[sc.Dim.Z].unit,
                         sc.units.dimensionless)
        # Sum over 2 dimensions to simplify finding max.
        max_1d = sc.sum(sc.sum(histo_dataarray, dim=sc.Dim.Y),
                        dim=sc.Dim.X).values
