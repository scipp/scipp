...
def test_pandas_1d_dataframe():
    pandas_df = pandas.Series(data=[1, 2, 3])

    sc_dataarray = from_pandas(pandas_df)

    assert (sc_dataarray.values == scipp.array(dims=["pandas_series"],
                                               values=[1, 2, 3]).values).all()
    assert sc_dataarray.dims == ["pandas_series"]


def test_pandas_1d_dataframe_with_named_axis():
    pandas_df = pandas.Series(data=[1, 2, 3])
...
def test_pandas_2d_dataframe_with_named_axes():
    pandas_df = pandas.DataFrame(data={1: (2, 3), 4: (5, 6)})
