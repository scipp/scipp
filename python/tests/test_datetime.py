...
def test_construct_0d_datetime_nounit():
    ...
    var.value = value
    assert sc.isequal(var, sc.Variable(value=value))
...
def test_0d_datetime_setter(unit):
    ...
    assert sc.isequal(var, sc.Variable(value=replacement))
...
def test_construct_datetime_nounit():
    ...
    var.values = values
    assert sc.isequal(var, sc.Variable(dims=['x'], values=values))
...
def test_datetime_setter(unit):
    ...
    assert sc.isequal(var, sc.Variable(dims=['x'], values=replacement))
...
def test_datetime_slicing(unit):
    ...
    for i in range(len(values1)):
        assert sc.isequal(var['x', i], sc.Variable(value=values1[i]))
    for i in range(len(values1) - 2):
        for j in range(i + 1, len(values1)):
            assert sc.isequal(var['x', i:j],
                              sc.Variable(dims=['x'], values=values1[i:j]))

    for i in range(len(values1)):
        var['x', i] = values2[i] * sc.Unit(unit)
    assert sc.isequal(var, sc.Variable(dims=['x'], values=values2))
