...
def assert_isequal(plotarray, dataarray):
    assert sc.isequal(plotarray.data, dataarray.data)
    for key in dataarray.meta:
        assert sc.isequal(plotarray.meta[str(key)], dataarray.meta[key])
    for msk in dataarray.masks:
        assert sc.isequal(plotarray.masks[msk], dataarray.masks[msk])
...
def test_plot_array_view_ragged():
    ...
    pa = PlotArray(data=da.data, meta=da.coords)
    assert_isequal(pa['tof', 5:10], da['tof', 5:10])
...
def test_plot_array_view_of_view_index():
    da = make_dense_dataset(ndim=3)["Sample"]
    pa = PlotArray(data=da.data, meta=da.coords)
    assert_isequal(pa['x', 10]['y', 10], da['x', 10]['y', 10])


def test_plot_array_view_of_view_range():
    da = make_dense_dataset(ndim=3)["Sample"]
    pa = PlotArray(data=da.data, meta=da.coords)
    assert_isequal(pa['x', 5:10]['y', 5:10], da['x', 5:10]['y', 5:10])
