...
def test_type():
    variable_slice = sc.Variable(['x'], np.arange(1, 10,
                                                    dtype=float))['x', :]
    assert type(variable_slice) == sc.VariableView


def test_astype():
    variable_slice = sc.Variable(['x'],
...
def test_binary_operations():
    _a = sc.Variable(['x'], np.arange(1, 10, dtype=float))
    _b = sc.Variable(['x'], np.arange(1, 10, dtype=float))
    a = _a['x', :]
    b = _b['x', :]
...
def test_correct_temporaries():
    v = sc.Variable(['x'], values=np.arange(100.0))
    b = sc.sqrt(v)['x', 0:10]
