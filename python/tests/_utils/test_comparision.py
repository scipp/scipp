...
def test_wont_match_when_meta_size_unequal():
    ...
    with pytest.raises(RuntimeError):
        su.isnear(a, b, rtol=0, atol=1.0)
    # Raise nothing if we are ignoring differing parts
    su.isnear(a, b, rtol=0, atol=1.0, include_attrs=False)
...
def test_data_scalar_no_coords():
    ...
    assert su.isnear(a, a, rtol=0, atol=1e-14)
    b = a + 1.0 * sc.units.one
    assert su.isnear(a, b, rtol=0.0, atol=1.0)
    assert su.isnear(a, b, rtol=1.0, atol=0.0)
    assert not su.isnear(a, b, rtol=0, atol=0.9999)


def test_data_scalar_no_coords_no_data():
    a = sc.DataArray(data=sc.scalar(value=1))
    b = a.copy()
    assert su.isnear(a, b, rtol=0, atol=1e-14,
...
def test_scalar_with_coord():
    ...
    assert su.isnear(a, a, rtol=0.0, atol=1e-14)
    b = sc.DataArray(data=point, coords={'x': point + point})
    assert su.isnear(a, b, rtol=0.0, atol=1.0)
    assert not su.isnear(a, b, rtol=0.0, atol=0.9999)
...
def test_with_many_coords_and_attrs():
    ...
    assert su.isnear(a, b, rtol=0.0, atol=0.9999, include_attrs=False)
