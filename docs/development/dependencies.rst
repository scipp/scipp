...
- GitHub workflows define ``runs-on``, i.e., VM image versions.
  We are not using the ``*-latest`` images but instead rely on specific ones such as ``ubuntu-24.04`` or ``windows-2022``.
...
- `pixi.toml <https://github.com/scipp/scipp/blob/main/pixi.toml>`_ defines the full developer environment including C++ build tools, test dependencies, documentation tools, and static analysis.
  It replaces the previous ``docs/environments/developer.yml`` conda environment file and ``tox.ini`` task runner.
