...
def _make_shape_repr(var: Union[Variable, DataArray, Dataset, DataGroup]):
...
def _make_dataset_summary(ds: Dataset):
...
def _summarize_atomic_variable(var, name: str, depth: int = 0):
...
def _collapsible_summary(var: DataGroup, name_spaces: list):
    name = name_spaces[-1]
    parent_type = "scipp"
    ...
    subsection = _datagroup_detail(var, name_spaces)
...
def _datagroup_detail(dg: DataGroup, name_spaces: list = None):
    if name_spaces is None:
        name_spaces = []
    summary_rows = []
    for name, item in dg.items():
        if isinstance(item, DataGroup):
            collapsible_row = _collapsible_summary(item, name_spaces + [name])
...
def datagroup_repr(dg: DataGroup):
