...
class PlotFigure2d(PlotFigure):
    ...
    def __init__(
        ...
        self.image_colors = self.ax.imshow(
            self.cmap(self.norm(ones)), zorder=1, **image_params
        )

        self.image_values = self.ax.imshow(
            ones,
            norm=self.norm,
            cmap=self.cmap,
            picker=5,
        ...
        if cbar:
            self.cbar = plt.colorbar(
                self.image_values, ax=self.ax, cax=self.cax, extend=extend
            )
