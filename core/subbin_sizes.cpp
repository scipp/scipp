...
void SubbinSizes::trim_to(const SubbinSizes &other) {
  ...
  const auto end = std::min(offset() + scipp::size(sizes()), out.offset() + scipp::size(out.sizes()));
  
  const auto ibegin = sizes().begin() + std::clamp(begin - offset(), scipp::index(0), scipp::size(sizes()));
  
  const auto obegin = out.m_sizes.begin() + std::max(begin - out.offset(), scipp::index(0));
