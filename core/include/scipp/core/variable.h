...
namespace scipp::core {
  ...
  template <class T> struct element_type<sparse_container<T> > {
    using type = T;
  };
  ...
  public:
    ...
    DType dtype(bool sparse = false) const noexcept override {
      ...
      if
        constexpr(is_sparse_container<T>::value) return scipp::core::dtype<
    ...
    VariableConceptHandle makeDefaultFromParent(const Dimensions &dims) const
        override;
  ...
  public:
    ...
    template <class T> VariableConceptHandle_impl(T object) {
      ...
      if
        constexpr((std::is_same_v<value_t, Known> || ...)) m_object =
            std::unique_ptr<VariableConceptT<value_t> >(std::move(object));
      else
    ...
    VariableConceptHandle_impl(const VariableConceptHandle_impl &other)
        : VariableConceptHandle_impl(other ? other->clone()
                                           : VariableConceptHandle_impl()) {}
    VariableConceptHandle_impl &operator=(VariableConceptHandle_impl &&) =
        default;
    VariableConceptHandle_impl &operator=(
        const VariableConceptHandle_impl &other) {
    ...
    VariableConcept &operator*() const {
      return std::visit([](auto && arg)->VariableConcept & {
        return *arg;
      },
                        m_object);
    }
  ...
  private:
    std::variant<std::unique_ptr<VariableConcept>,
                 std::unique_ptr<VariableConceptT<Known> >...> m_object;
  };
  ...
  template <class T> constexpr bool is_container_or_proxy() {
    return std::is_same_v<T, Dataset> || std::is_same_v<T, DatasetProxy> ||
           std::is_same_v<T, DatasetConstProxy> ||
           std::is_same_v<T, Variable> || std::is_same_v<T, VariableProxy> ||
           std::is_same_v<T, VariableConstProxy> ||
  ...
  public:
    ...
    template <typename T,
              typename = std::enable_if_t< !is_variable_or_proxy<T>()> >
  ...
  private:
    template <class T>
    const Vector<T> &cast(const bool variances = false) const;
...
Variable makeVariableInit(const Dimensions &dimensions,
  ...
  template <class T>
  Variable makeVariableWithVariances(
  ...
  template <class T> Variable makeVariable(T value, T variance) {
    return Variable(units::dimensionless, Dimensions {
                    ...
                    Vector<T>(1, value), Vector<T>(1, variance));
  }
  ...
  template <class... Ts>
  Variable Variable::ConstructVariable<Ts...>::make(Ts && ... args,
                                                    DType type) {
    return CallDTypeWithSparse<double, float, int64_t, int32_t, bool,
                               Eigen::Vector3d, std::string>::apply<Maker>(
  ...
  Variable makeVariable(const Dimensions & dimensions, Args && ... args) {
    ...
    if
      ...
      constexpr(sizeof...(Args) == 2) {
        if
          constexpr(std::is_convertible_v<detail::nth_t<0, Args...>,
                                          std::vector<T> > &&
      ...
      constexpr(sizeof...(Args) == 3) {
        if
          constexpr(
              std::is_convertible_v<detail::nth_t<0, Args...>, units::Unit> &&
              std::is_convertible_v<detail::nth_t<1, Args...>,
                                    std::vector<T> > &&
  ...
  template <typename T>
  std::enable_if_t<std::is_arithmetic_v<T>, Variable> operator*(
      T v, const units::Unit & unit) {
  ...
  std::enable_if_t<std::is_arithmetic_v<T>,
                   Variable> operator/(T v, const units::Unit & unit) {
    return makeVariable<T>({
    },
                           units::Unit(units::dimensionless) / unit, {
      v
    });
  }
  ...
  SCIPP_CORE_EXPORT std::vector<Variable> split(
  ...
  SCIPP_CORE_EXPORT Variable filter(const Variable & var,
                                    const Variable & filter);
  SCIPP_CORE_EXPORT Variable mean(const VariableConstProxy & var,
                                  const Dim dim);
  ...
  SCIPP_CORE_EXPORT Variable permute(const Variable & var, const Dim dim,
                                     const std::vector<scipp::index> & indices);
  SCIPP_CORE_EXPORT Variable rebin(
      const VariableConstProxy & var, const Dim dim,
      const VariableConstProxy & oldCoord, const VariableConstProxy & newCoord);
  SCIPP_CORE_EXPORT Variable resize(const VariableConstProxy & var,
