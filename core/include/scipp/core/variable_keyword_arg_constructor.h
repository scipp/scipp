...
namespace scipp::core {
  ...
  // Structures Values and Variances just forwards the arguments for
  // constructing
  ...
  template <class U> struct vector_like {
    std::vector<U> data;
    template <class... Args>
    vector_like(Args &&... args)
        : data(std::forward<Args>(args)...) {}
    template <class T>
    vector_like(std::initializer_list<T> init)
  ...
  template <class... Args>
  struct Variances : std::tuple<Args...>,
                     detail::VariancesTag {
    ArgsTuple<Args...> tuple;
    Variances(Args &&... args)
        : tuple(detail::makeArgsTuple(std::forward<Args>(args)...)) {}
  ...
  template <class T, class... Args> class Indexer {
    ...
    static constexpr auto indexOfCorresponding_impl(
...
public:
  template <class... NonDataTypes> constexpr static bool checkArgTypesValid() {
    ...
    static VarT construct(std::tuple<ValArgs...> &&valArgs,
      ...
      if
        constexpr((hasVal && !constrVal) ||
                  (hasVar && !constrVar)) throw except::TypeError(
            "Can't create the Variable with type " +
  ...
  private:
    ...
    static auto extractArgs(std::tuple<Args...> &tp) {
      if
        constexpr(!is_type_in_pack_v<T, Ts...>)return T {}
      ;
      else return std::move(std::get<T>(tp));
    }

    template <class Tag, class... Args>
    static auto extractTagged(std::tuple<Args...> &tp) {
      if
        constexpr(!is_tag_in_pack_v<Tag, Ts...>)return std::tuple {}
      ;
      else {
