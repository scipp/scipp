...
def test_create_scalar_with_float_value(value) -> None:
...
def test_create_scalar_with_float_variance(variance) -> None:
...
def test_create_scalar_with_float_value_and_variance(value, variance) -> None:
...
def test_create_scalar_with_value(args) -> None:
    dtype, value = args
    var = sc.Variable(dims=(), values=value)
    assert var.value == value
    assert var.dims == ()
...
def test_create_scalar_with_value_array(args) -> None:
    dtype, value = args
...
def test_create_scalar_invalid_variance(variance) -> None:
...
def test_create_scalar_with_unit(unit) -> None:
...
def test_create_scalar_dtype_DataArray(dtype) -> None:
...
def test_create_scalar_dtype_Dataset(dtype) -> None:
...
def test_create_scalar_with_instance_as_dtype(dtype) -> None:
    with pytest.raises(TypeError):
        sc.Variable(dims=(), values=dtype, dtype=dtype)


@pytest.mark.parametrize("dtype", [str, sc.Variable, sc.DataArray, sc.Dataset])
def test_create_scalar_value_must_be_convertible_to_dtype(dtype) -> None:
...
def test_create_scalar_conversion(value, dtype) -> None:
...
def test_create_scalar_invalid_conversion_numeric(value, dtype) -> None:
    with pytest.raises(ValueError, match='Cannot convert values'):
...
def test_create_scalar_invalid_conversion_str(dtype) -> None:
    with pytest.raises(ValueError, match='Cannot convert values'):
...
def test_create_1d_dtype(values_type, dtype_and_values) -> None:
    def check(v):
...
@pytest.mark.parametrize('values_type', [tuple, list, np.array])
def test_create_1d_values_array_like(values_type) -> None:
...
def test_create_1d_variances_array_like(variances_type) -> None:
...
@pytest.mark.parametrize('values_type', [tuple, list, np.array])
@pytest.mark.parametrize('variances_type', [tuple, list, np.array])
def test_create_1d_values_and_variances_array_like(values_type, variances_type) -> None:
    values = np.arange(5.0)
...
def test_create_1d_override_dtype(dtype, values) -> None:
    var = sc.Variable(dims=['x'], values=values, dtype=dtype)
...
def test_create_2d_values_array_like(inner_values_type, outer_values_type) -> None:
...
def test_create_2d_variances_array_like(
    inner_variances_type, outer_variances_type
) -> None:
