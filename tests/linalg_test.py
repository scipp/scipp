...
def test_variable_0D_matrix():
    ...
    rot = sc.matrix(
        value=sc.geometry.rotation_matrix_from_quaternion_coeffs([0, 0, 1, 0]),
        unit=sc.units.one,
    )
    vec = sc.vector(value=[1, 2, 3], unit=sc.units.m)
...
def test_set_matrices_values():
    var = sc.matrices(
        dims=['x'], values=np.arange(18).reshape(2, 3, 3), unit=sc.units.m
    )
...
def test_matrix_elements():
    ...
    for elem, offset in zip(elems, offsets):
        assert sc.identical(
            elem,
            sc.array(dims=['x'], values=np.array([0.0, 9.0]) + offset, unit=sc.units.m),
        )
...
def test_matrix_elements_setter():
    ...
    m.fields.xx = sc.array(dims=['x'], values=np.array([1.0, 1.0]), unit=sc.units.m)
    ...
    m.fields.yz = sc.array(dims=['x'], values=np.array([1.0, 1.0]), unit=sc.units.m)
    assert not sc.identical(m, expected)
    values[:, 1, 2] = 1
    expected = sc.matrices(dims=['x'], values=values, unit=sc.units.m)
    assert sc.identical(m, expected)
...
def test_elements_binned():
    ...
    var = sc.bins(dim='x', data=data)
    assert var.fields is not None
    yz = sc.bins(dim='x', data=sc.array(dims=['x'], values=[5.0, 14.0]))
